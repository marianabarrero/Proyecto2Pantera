...
// --- Iconos Personalizados para Usuarios ---
const user1Icon = new Icon({ iconUrl: "/icon.png", iconSize: [70, 70] });
const user2Icon = new Icon({ iconUrl: "/logo.png", iconSize: [70, 70] }); // Asegúrate de que este ícono exista en tu carpeta 'public'
...
const DateSearchModal = ({ isOpen, onClose, onSearch }) => {
  ...
  const darkTheme = createTheme({
    palette: {
      mode: 'dark',
      primary: { main: '#0d47a1' },
      background: { paper: '#1a237e' },
      text: { primary: '#FFFFFF', secondary: '#E5E7EB' },
      typography: { fontFamily: 'Poppins, sans-serif' },
    },
    components: {
      MuiPickersToolbar: { styleOverrides: { root: { backgroundColor: 'rgba(13, 71, 161, 0.2)' } } },
      MuiDialogActions: { styleOverrides: { root: { '& .MuiButton-root': { color: '#FFFFFF' } } } },
    },
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative glassmorphism-strong rounded-4xl p-8 mx-4 w-full max-w-5xl transform">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={onClose} className="text-white/60 cursor-pointer hover:text-white p-1"></button>
        </div>
        ...
        <div className="flex gap-4 pt-6 mt-4 border-t border-white/20">
          <button onClick={resetForm} className="flex-1 px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all font-medium" disabled={isLoading}>
            Clean
          </button>
          <button onClick={handleSearch} disabled={isLoading || !startDate || !endDate} className="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-sky-600 to-sky-700 hover:from-sky-700 hover:to-sky-800 text-white rounded-xl transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed">
...
// --- COMPONENTE DE MAPA MODIFICADO ---
const LocationMap = ({ locations, users, formatTimestamp, path }) => {
  const position = locations.length > 0
    ? [parseFloat(locations[0].latitude), parseFloat(locations[0].longitude)]
    : [4.60971, -74.08175]; // Coordenadas por defecto si no hay datos
  ...
  return (
    <div className='glassmorphism-strong rounded-4xl backdrop-blur-lg shadow-lg p-4 max-w-4xl w-full mx-4'>
      <MapContainer center={position} zoom={18} style={{ height: '45rem', width: '100%', borderRadius: '1rem' }}>
        <TileLayer
        ...
        {locations.map((loc) => (
          <Marker
            ...
            icon={userIcons[loc.user_id] || new Icon({ iconUrl: "/icon.png", iconSize: [70, 70] })}
          >
            <Popup>
              <div className="text-center">
                <strong>{users[loc.user_id]?.name || 'Unknown User'}</strong><br />
                <small>Received: {formatTimestamp(loc.timestamp_value)}</small>
              </div>
            </Popup>
          </Marker>
        ))}
        <Polyline pathOptions={polylineOptions} positions={path} />
        {locations.length === 1 && <MapUpdater position={position} />}
      </MapContainer>
    </div>
  );
};
...
function App() {
  ...
  const [allLocations, setAllLocations] = useState([]); // <-- Nuevo estado
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [path, setPath] = useState([]);
  const [isDateSearchModalOpen, setIsDateSearchModalOpen] = useState(false);
  const [isLiveMode, setIsLiveMode] = useState(true);
  const [currentUser, setCurrentUser] = useState('user_1'); // Puede ser 'user_1', 'user_2', o 'all'
  const [users, setUsers] = useState({
    user_1: { name: 'User 1', color: '#110394' },
    user_2: { name: 'User 2', color: '#940311' }
  });
  ...
  const fetchLatestLocation = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setAllLocations([]); // Limpia el estado de múltiples usuarios
  ...
  const fetchAllLatestLocations = async () => {
    try {
      const response = await fetch(`${config.API_BASE_URL}/api/location/latest/all`);
      if (!response.ok) throw new Error('Error fetching all locations');

      const data = await response.json();
      setAllLocations(data);
      setLocationData(null); // Limpia el estado de un solo usuario
      setError(null);
    } catch (err) {
      setError('Could not fetch data for all users.');
  ...
  const handleDateSearch = async (searchData) => {
    ...
    try {
      ...
      if (historicalData.length > 0) {
        const newPath = historicalData.map(point => [parseFloat(point.latitude), parseFloat(point.longitude)]);
        setPath(newPath);
  ...
  useEffect(() => {
    ...
    if (isLiveMode) {
      if (currentUser === 'all') {
        ...
        interval = setInterval(fetchAllLatestLocations, config.POLLING_INTERVAL);
      } else {
        fetchLatestLocation();
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="w-full md:w-3/4 animate-slide-in-left interactive-glow rounded-4xl">
              <LocationMap
                locations={currentUser === 'all' ? allLocations : (locationData ? [locationData] : [])}
                users={users}
                formatTimestamp={formatTimestamp}
                path={currentUser !== 'all' ? path : []}
              />
            </div>
              ...
              <div className="flex justify-center gap-4">
                {Object.keys(users).map(userId => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-colors ${currentUser === userId ? 'bg-sky-600 text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}
                  >
                    {users[userId].name}
                  </button>
                ))}
                <button
                  ...
                  className={`px-4 py-2 rounded-lg transition-colors ${currentUser === 'all' ? 'bg-sky-600 text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}
                >
                  Show All
                </button>
              </div>
              {currentUser !== 'all' && locationData ? (
                <LocationInfo
                ...
                currentUser === 'all' && (
                  <div className="glassmorphism-strong rounded-4xl p-8 text-center">
                    <h2 className='text-2xl font-bold text-white mb-4'>All Users</h2>
                    <p className="text-white/70">
                      Showing the latest location for all active users.
                    </p>
                  </div>
                )
              )}
